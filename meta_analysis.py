...
Lee 2021, 0.23, 0.028""", key="meta_paste")
        ...
        if studies:
            ...
            st.dataframe(studies_df, width='stretch', hide_index=True,
                        column_config=col_config)

            col1, col2 = st.columns(2)
            with col1:
                if st.button("ðŸ—‘ï¸ Clear All Studies", width='stretch'):
                    st.session_state["meta_studies"] = []
                    st.session_state["meta_results"] = None
                    st.rerun()
            with col2:
                if st.button("ðŸš€ Run Meta-Analysis", type="primary", width='stretch'):
                    if len(studies) >= 2:
        ...
        if not results:
            ...
            if forest_data:
                ...
                with st.expander("ðŸ“‹ Forest Plot Data"):
                    ...
                    st.dataframe(forest_df, width='stretch', hide_index=True)
            else:
    ...
    with tab5:
        ...
        if not raw_effects or len(raw_effects) < 2:
            ...
            with st.expander("ðŸ“ˆ Cumulative Meta-Analysis", expanded=False):
                ...
                if st.button(" Run Cumulative Analysis", width='stretch'):
            ...
            with st.expander("ðŸ” Leave-One-Out Sensitivity Analysis", expanded=False):
                ...
                if len(raw_effects) >= 3:
                    if st.button("ðŸ”¬ Run Sensitivity Analysis", width='stretch'):
                        ...
                        st.dataframe(loo_df, width='stretch', hide_index=True)
            ...
            with st.expander("ðŸ“ Meta-Regression", expanded=False):
                ...
                if st.button("ðŸ“ Run Meta-Regression") and len(mod_values) == n_studies:
                    ...
                    if "error" in mr:
                        ...
                        st.dataframe(coeff_df, width='stretch', hide_index=True)
