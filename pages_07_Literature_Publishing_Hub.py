...
def render_publication_pipeline():
    ...
    for i, (title, desc) in enumerate(steps, 1):
        st.markdown(
            f"""<div style="display:flex; gap:1rem; align-items:center; background:{t['bg_card']}; border:1px solid {t['border']}; border-radius:10px; padding:1.0rem 1.2rem; margin-bottom:0.75rem;">
                <div style="background:{t['accent']}22; color:{t['accent']}; border-radius:50%; width:36px; height:36px; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:1.1rem;">{i}</div>
                <div><div style="font-weight:700; color:{t['text_primary']}; font-size:1.05rem;">{title}</div><div style="color:{t['text_muted']}; font-size:0.9rem;">{desc}</div></div>
            </div>""",
