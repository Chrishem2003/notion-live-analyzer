...
install permanent" â€” the durable fix is requirements.txt, checked at
...
2. Offers a real "Install Now" button that runs an actual subprocess pip
   install into the current running process â€” this genuinely works on
...
def render_environment_manager():
    st.markdown("#### ðŸ“¦ Dependency Status")
    st.caption(
        ...
        "*this running session only* â€” it will not survive a restart unless requirements.txt is also "
    ...
    if st.button(f"âš¡ Install {len(missing)}} missing package(s) now (this session only)", type="primary"):
        ...
        st.info(
            ...
            "permanent â€” otherwise this reverts the next time the app restarts."
        )
