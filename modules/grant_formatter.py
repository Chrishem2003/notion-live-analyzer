...
def render_grant_formatter_ui():
    ...
    with tab1:
        ...
        if st.button("ðŸ”„ Reformat", type="primary", width='stretch') and content:
            ...
            st.download_button("ðŸ“¥ Download", result["content"], file_name=f"{target.lower().replace(' ', '_')}.md", mime="text/markdown", width='stretch')
    ...
    with tab2:
        ...
        if lit_papers:
            ...
            if st.button("ðŸ“š Format References", type="primary", width='stretch'):
                refs = formatter.format_references(lit_papers, target_ref)
                st.markdown(refs)
                st.download_button("ðŸ“¥ Download References", refs, file_name=f"references_{target_ref.lower().replace(' ', '_')}.md", mime="text/markdown", width='stretch')
        else:
