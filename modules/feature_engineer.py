...
def render_feature_engineering_ui():
    ...
    with tab1:
        ...
        if st.button("ðŸ” Discover Interactions", type="primary", width='stretch'):
    ...
    with tab2:
        ...
        if st.button("ðŸ“ Generate Polynomials", type="primary", width='stretch') and poly_cols:
            ...
            st.dataframe(result[new_features].head(10), width='stretch')
    ...
    with tab3:
        ...
        if st.button(" Apply Binning", type="primary", width='stretch'):
            binned = engine.auto_bin(df, bin_col, bin_method, n_bins)
            st.success(f"âœ… Binned into {binned.nunique()} bins")
            st.dataframe(binned.value_counts().reset_index(), width='stretch')
    ...
    with tab4:
        ...
        if st.button("ðŸ“… Extract Date Features", type="primary", width='stretch'):
    ...
    with tab5:
        ...
        if st.button("ðŸŽ¯ Run Feature Selection", type="primary", width='stretch'):
