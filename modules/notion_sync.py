...
def render_notion_sync_ui(df: pd.DataFrame):
    ...
    tab1, tab2, tab3 = st.tabs(["📤 Push Insights", "📝 Push Cleaned Data", "📋 Sync History"])

    with tab1:
        st.subheader("📤 Push AI Insights to Notion")
        ...
        if insights:
            ...
            if st.button("🚀 Push Insight to Notion Pages", type="primary"):
    ...
    with tab2:
        st.subheader("📝 Push Cleaned/Transformed Data")
        ...
        if st.button("🔄 Sync Column to Notion", type="primary"):
        ...
        with col2:
            if st.button("🏷️ Sync Tags/Notes", use_container_width=True):
