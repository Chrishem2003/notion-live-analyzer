...
def render_google_sheets_ui(df: pd.DataFrame):
                ...
                if worksheet_names:
                    ...
                    if st.button("ðŸ“¥ Read from Sheet", type="primary"):
                        ...
                        if read_df is not None and not read_df.empty:
                            st.success(f"âœ… Read {len(read_df)} rows ÃƒÆ’â€” {len(read_df.columns)} columns")
                            st.dataframe(read_df.head(50), width='stretch', hide_index=True)

                            # Option to use as active data
                            if st.button(" Use This Data for Analysis", width='stretch'):
    ...
    with tab2:
        ...
        if df is not None and not df.empty:
            st.info(f"**Data to write**: {len(df)} rows ÃƒÆ’â€” {len(df.columns)} columns from '{st.session_state.get('data_source', 'active dataset')}'")
