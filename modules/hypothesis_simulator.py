...
def render_hypothesis_simulator_ui():
    ...
    sim_type = st.radio("Select simulation type", [
        " Power Analysis", "Ã°Å¸â€™Å  Dose-Response", "Ã°Å¸â€â€” Correlation",
        "Ã°Å¸â€œÂ Confidence Intervals", "ðŸ“ˆ Regression", "Ã¢Å¡Â Ã¯Â¸Â Bias Impact"
    ...
    if sim_type == " Power Analysis":
        ...
        with col1:
            ...
            alpha = st.select_slider("Alpha (ÃŽÂ±)", options=[0.001, 0.01, 0.05, 0.10], value=0.05, key="sim_power_alpha")
        with col2:
            power_target = st.slider("Target power (1-ÃŽÂ²)", 0.50, 0.99, 0.80, 0.05, key="sim_power_target")
            n_current = st.slider("Current N per group", 5, 500, 30, 5, key="sim_power_n")

        if st.button("Ã¢â€“Â¶Ã¯Â¸Â Run Power Simulation", type="primary"):
    ...
    elif sim_type == "Ã°Å¸â€™Å  Dose-Response":
        st.subheader("Ã°Å¸â€™Å  Dose-Response Curve Simulator")
        col1, col2 = st.columns(2)
        with col1:
        ...
        if st.button("Ã¢â€“Â¶Ã¯Â¸Â Run Dose-Response Simulation", type="primary"):
    ...
    elif sim_type == "Ã°Å¸â€â€” Correlation":
        st.subheader("Ã°Å¸â€â€” Correlation Simulator")
        col1, col2 = st.columns(2)
        with col1:
            n_corr = st.slider("Sample size (N)", 10, 500, 100, 10, key="sim_corr_n")
        with col2:
            r_true = st.slider("True correlation (ÃÂ)", -1.0, 1.0, 0.5, 0.05, key="sim_corr_r")
        ...
        st.subheader("Ã°Å¸â€œÂ Confidence Interval Coverage Simulator")
        col1, col2 = st.columns(2)
        with col1:
        ...
        with col1:
            ...
            b0 = st.number_input("True intercept (ÃŽÂ²Ã¢â€šâ‚¬)", value=0.0, step=0.5, key="sim_reg_b0")
        with col2:
            b1 = st.number_input("True slope (ÃŽÂ²Ã¢â€šÂ)", value=1.5, step=0.5, key="sim_reg_b1")
            noise = st.slider("Noise (ÃÆ’)", 0.1, 5.0, 1.0, 0.1, key="sim_reg_noise")
        ...
        st.subheader("Ã¢Å¡Â Ã¯Â¸Â Publication Bias Simulator")
        col1, col2 = st.columns(2)
        with col1:
