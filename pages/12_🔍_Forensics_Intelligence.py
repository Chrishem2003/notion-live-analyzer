...
def render_evidence_lab_tab() -> None:
    section_header(
        "💼 Bit-Level Digital Evidence Laboratory", 
        "Deep binary inspection, magic byte verification, entropy profiling, cryptographic hashing, and automated per-evidence chain-of-custody logging."
    )
    ...
    ext = report.get("extension_check", {}) or {}
...
def render_metadata_tab() -> None:
    section_header(
        "🖼️ Deep Metadata & EXIF Geolocation Forensics", 
        "Extract hidden EXIF tags, device manufacturer signatures, original capture timestamps, and interactive GPS coordinate mapping."
    )
    ...
    if exif.get("has_exif"):
        ...
        col1.metric("Camera Make", str(exif.get("Make", "—")))
        col2.metric("Camera Model", str(exif.get("Model", "—")))
        col3.metric("Software Tool", str(exif.get("Software", "—")))
        ...
        if isinstance(gps, dict):
            ...
            if lat is not None and lon is not None:
                try:
                    ...
                    st.error("🚨 Malformed GPS floating-point coordinates encountered.")
            else:
                st.info("ℹ️ GPS dictionary present, but valid coordinate values were not found.")
        else:
...
def render_stego_tab() -> None:
    section_header(
        "🧩 LSB Steganography & Bitstream Anomaly Detector", 
        "Mathematically analyze least-significant bit (LSB) variance, chi-square pixel distributions, and bitstream entropy to uncover hidden payloads."
    )
    ...
    if result.get("supported"):
        ...
        likelihood = str(result.get("hidden_payload_likelihood", "LOW")).upper()
        ...
        if result.get("estimate"):
            st.write(result["estimate"])
    else:
        st.warning(result.get("note", "Steganographic analysis is unsupported for this file format."))


def render_phishing_tab() -> None:
    section_header(
        "📧 SMTP Header & Email Phishing Forensics", 
        "Inspect raw RFC 5322 email headers, evaluate DKIM/SPF alignment flags, trace transmission hop relays, and detect domain spoofing."
    )
    ...
    if raw_email and st.button("🔍 Execute Email Forensic Analysis", key="fe_analyze_email_upg", type="primary"):
        ...
        cols = st.columns(3)
        cols[0].metric("From Domain", str(result.get("from_domain", "—")))
        cols[1].metric("Reply-To Domain", str(result.get("reply_to_domain", "—")))
...
def render_custody_tab() -> None:
    section_header(
        "🔗 Cryptographic Chain-of-Custody Vault", 
        "Court-admissible tamper-evident ledger where every investigative action is cryptographically chained via SHA-256 blocks — one independent chain per piece of evidence."
    )
