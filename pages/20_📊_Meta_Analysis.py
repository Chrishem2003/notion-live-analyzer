"""
Meta-Analysis Engine [ENTERPRISE ULTRA v6.0]
...
if "custom_access_password" not in st.session_state:
    # Key configured via session state configuration
...
hero_card(
    ...
    "Perform rigorous statistical pooling, multi-estimator heterogeneity modeling (DL, REML, HE), "
    "publication bias adjustments (Egger's Test & Trim-and-Fill), and interactive visualization.",
    badge_text="⚡ v6.0 ULTRA — Dynamic Effect Converters & Trim-and-Fill Suite"
)
watermark("ENTERPRISE_SYNTHESIS")

st.markdown("---")

# ─── Security Gate & Workspace Header ────────────────────────────────
col_sec, col_proj = st.columns([1, 1])

with col_sec:
    st.markdown("### 🔐 Security Authentication Gate")
    if not st.session_state.meta_engine_clearance:
        st.info("🔒 Enter your **Enterprise Security Passkey** to access analytical algorithms.")
...
def trim_and_fill(effects, std_errs, max_iter=20):
    ...
    for _ in range(max_iter):
        ...
        # Estimate number of missing studies on side with lower effects
...
# ───────────────────────────────────────────────────────────────────────
with tab1:
    section_header("📋 Study Data & Effect Size Converter Engine")
...
# ───────────────────────────────────────────────────────────────────────
with tab2:
    section_header("⚙️ Advanced Model Pooling & Variance Estimators")
    ...
    else:  # DL default / REML approximation fallback
...
# ───────────────────────────────────────────────────────────────────────
with tab3:
    section_header("🌲 Interactive Forest Plot")

    fig = go.Figure()

    # Individual Study Points & CIs
    for i in range(k):
        es = effect_sizes[i]
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='lines+markers',
            marker=dict(size=[6, 10, 6], symbol=['line-ew', 'square', 'line-ew'], color='navy'),
    ...
    diamond_x = [ci_low, pooled_active, ci_high, pooled_active, ci_low]
    diamond_y = ["Overall Pooled", "Overall Pooled (Top)", "Overall Pooled", "Overall Pooled (Bottom)", "Overall Pooled"]
    
    fig.add_trace(go.Scatter(
        x=[ci_low, pooled_active, ci_high],
        y=["Overall Pooled", "Overall Pooled", "Overall Pooled"],
        mode='lines+markers',
        marker=dict(size=[8, 12, 8], symbol=['diamond', 'diamond', 'diamond'], color='firebrick'),
    ...
    # Null Reference Line
    fig.add_vline(x=0, line_dash="dash", line_color="gray", annotation_text="Null Effect")
    fig.add_vline(x=pooled_active, line_dash="dot", line_color="firebrick")
...
# ───────────────────────────────────────────────────────────────────────
with tab4:
    section_header("📉 Publication Bias Diagnostics & Trim-and-Fill")
    ...
    if k < 3:
        ...
        fig_funnel.add_trace(go.Scatter(
            ...
            mode='markers',
            marker=dict(size=9, color='royalblue', line=dict(width=1, color='black')),
        ...
        if k0 > 0:
            ...
            fig_funnel.add_trace(go.Scatter(
                ...
                mode='markers',
                marker=dict(size=9, color='orange', symbol='circle-open', line=dict(width=2, color='darkorange')),
                name='Imputed Studies (Trim-and-Fill)'
            ))

        # Pseudo 95% Confidence Cone
        se_seq = np.linspace(0.001, max(filled_se) * 1.15, 100)
        cone_upper = pooled_active + 1.96 * se_seq
        cone_lower = pooled_active - 1.96 * se_seq

        fig_funnel.add_trace(go.Scatter(x=cone_upper, y=se_seq, mode='lines', line=dict(color='gray', dash='dash'), showlegend=False))
        fig_funnel.add_trace(go.Scatter(x=cone_lower, y=se_seq, mode='lines', line=dict(color='gray', dash='dash'), showlegend=False))
        fig_funnel.add_vline(x=pooled_active, line_color='firebrick', annotation_text="Pooled ES")

        fig_funnel.update_layout(
            title="Funnel Plot with Pseudo 95% Confidence Limits",
            xaxis_title="Effect Size",
            yaxis_title="Standard Error (SE)",
            yaxis=dict(autorange="reversed"),  # Invert SE axis per standard convention
            template="plotly_white",
            height=500
        )
...
# ───────────────────────────────────────────────────────────────────────
with tab5:
    section_header("🔬 Statistical Heterogeneity & Subgroup Stratification")
