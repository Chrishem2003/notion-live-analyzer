...
st.set_page_config(page_title="Threat Scanner Suite", page_icon="☣️", layout="wide")
...
"""
 Threat & Scanner Suite â€” Enterprise Production Grade (Fully Functional)
...
def render_pii_tab():
    ...
    if source_mode == "Active Session Dataset":
        ...
        if df is None:
            st.info("â„¹ No active dataset loaded in this session â€” load one via Data Studio, or switch to 'Upload New File' / 'Paste Raw Text' above.")
        else:
        ...
        if uploaded is not None:
            try:
                ...
                st.error(f"âš  Failed to parse uploaded file: {e}")
    else:
    ...
    if st.button(" Execute Comprehensive PII Scan", key="ts_pii_run_upg", type="primary"):
        ...
        if "CRITICAL" in overall:
            st.error(f" **{overall}** â€” Detected {result.get('total_matches', 0):,} severe compliance violations.")
        elif "REVIEW" in overall:
            st.warning(f"âš  **{overall}** â€” Potential exposure vectors identified.")
        else:
        ...
        if result.get("findings"):
            ...
            for f in result["findings"]:
                with st.expander(f"âš  {f.get('type', 'Unknown')} â€” {f.get('count', 0)} match(es) [Risk Level: {f.get('risk', 'MODERATE')}]"):
...
def render_yara_tab():
    section_header(" YARA-Lite Malware & Exploit Signature Scanner", "Scan uploaded evidentiary artifacts against advanced malware signatures, ransomware strings, and exploit payloads.")

    uploaded = st.file_uploader("Upload binary artifact for signature inspection", key="ts_yara_upload_upg")
    if uploaded is None:
        st.info("â„¹ Upload a file to execute signature scanning.")
        return
...
def render_integrity_tab():
    ...
    with col_a:
        if st.button(" Establish Cryptographic Baseline", key="ts_int_create_upg", type="primary"):
            ...
            if missing:
                st.warning(f"âš  These paths don't exist from the app's working directory and will be skipped: {', '.join(missing)}")
    ...
    with col_b:
        if st.button("âœ… Verify File Integrity Ledger", key="ts_int_verify_upg", type="primary"):
            ...
            if result.get("changed_or_deleted", 0) == 0:
                st.success(f" **{result.get('verdict', 'INTEGRITY VERIFIED')}** â€” `{result.get('verified_unchanged', 0)}` files verified unchanged.")
            else:
                st.warning(f"âš  **{result.get('verdict', 'MODIFICATIONS DETECTED')}**")
...
def render_port_tab():
    ...
    st.warning("âš  **Strict Authorization Notice:** Ethical use only. Only scan systems, IP addresses, or infrastructure you own or possess explicit written authorization to test.")
...
def render_threat_tab():
    section_header(" Global Threat Intelligence Hub", "Cross-reference IP reputation feeds, extract domain WHOIS registration intelligence, and run heuristic phishing URL analysis.")

    tab_ip, tab_domain, tab_url = st.tabs([" IP Reputation", " Domain WHOIS", " URL Phishing Analyzer"])
    ...
    with tab_ip:
        ...
        if st.button(" Query IP Reputation", key="ts_threat_ip_run_upg", type="primary"):
            ...
            if risk == "HIGH":
                st.error(f" Threat Risk Assessment: **{risk}** â€” Abuse Confidence Score: `{result.get('abuse_confidence', 0)}%`")
            elif risk == "MEDIUM":
                st.warning(f"âš  Threat Risk Assessment: **{risk}** â€” Abuse Confidence Score: `{result.get('abuse_confidence', 0)}%`")
            else:
    ...
    with tab_url:
        ...
        if st.button(" Analyze URL Phishing Vectors", key="ts_threat_url_run_upg", type="primary"):
            ...
            risk = result.get("risk", "LOW")
            if risk == "HIGH":
                st.error(f" **{result.get('verdict', 'Malicious URL')}** â€” Risk Score: `{result.get('risk_score', 0)} / 100`")
            elif risk == "MEDIUM":
                st.warning(f"âš  **{result.get('verdict', 'Suspicious URL')}**")
            else:
...
def main():
    ...
    setup_page("Threat & Scanner Suite", " initial_sidebar_state="expanded")
    ...
    hero_card(
        " Threat & Scanner Suite â€” Premium Security Operations",
