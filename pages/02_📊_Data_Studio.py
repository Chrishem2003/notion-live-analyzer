...
def apply_recipe_step(df: pd.DataFrame, step: dict) -> pd.DataFrame:
    ...
    if kind == "Compute Feature":
        ...
        if col1 not in out.columns or col2 not in out.columns:
            raise KeyError(f"Replay requires columns '{col1}' and '{col2}'.")
        if "+" in op:
    ...
    if kind == "Quantile Binning":
        ...
        if col not in out.columns:
            raise KeyError(f"Replay requires column '{col}'.")
...
def render_ingestion_tab():
    section_header("📥 Enterprise Data Ingestion Hub", "Secure ingestion pipeline supporting multi-format files and sample generators.")
        ...
        if uploaded_file is not None:
            ...
            with st.spinner(f"Parsing and running structural validation on '{uploaded_file.name}'..."):
                ...
                if df is not None and not df.empty:
                    ...
                    if prev_fp == new_fp:
                        st.warning("⚠️ This file is byte-for-byte identical to the currently active dataset.")
                    elif prev_sig:
                    ...
                    st.success(f"✅ Ingested `{uploaded_file.name}` — {df.shape[0]:,} rows × {df.shape[1]} columns")
...
def render_quality_tab():
    ...
    section_header("🔍 Enterprise Quality Audit & Remediation", "Deep anomaly scanning, missingness analysis, and data cleaning.")
...
def render_transform_tab():
    df = get_active_dataframe()
    section_header("⚙️ Enterprise Transform Studio", "Execute calculations, binning, scaling protocols, and recipe logging.")
...
def render_variable_editor_tab():
    df = get_active_dataframe()
    section_header("🏷️ Enterprise Schema & Metadata Manager", "Manage variable types, labels, and scales.")
...
def render_simulator_tab():
    section_header("🎲 Enterprise Synthetic Data Simulator", "Generate statistically controlled synthetic datasets.")
...
def validate_readonly_sql(query: str):
    ...
    # Precise word boundary matching to allow valid SQL clauses like "ORDER BY created_at"
    upper_q = stripped.upper()
    for kw in FORBIDDEN_SQL_KEYWORDS:
        if re.search(rf"\b{kw}\b", upper_q):
            return False, f"Statement contains a restricted keyword: `{kw}`."
    return True, stripped


def render_explorer_tab():
    df = get_active_dataframe()
    section_header("📋 Dataset Explorer & Enterprise Export", "Inspect, query via DuckDB sandbox, and export assets.")
    ...
    with tab_sql:
        if DUCKDB_AVAILABLE:
            ...
            st.caption(f"SELECT-only. Results capped at {SQL_RESULT_ROW_CAP:,} rows.")
...
def main():
    ...
    hero_card(
        ...
        "Production-grade data management hub featuring secure schema management, sandboxed transformations, a replayable recipe pipeline, dataset fingerprinting, and anomaly detection.",
