...
.nv-card {{
    ...
    border: 1px solid rgba(201,154,58,0.20); 
    border-radius: 14px; 
    padding: 16px 18px;
}}
...
.stButton>button {{
    ...
    box-shadow: 0 3px 10px rgba(201,154,58,0.25);
}}
        ...
        if deck:
            ...
            for i, s in enumerate(slides):
                with st.container(border=True):
                    s["title"] = st.text_input(f"Slide {i} title", value=s.get("title", ""), key=f"st_{deck['id']}_{i}")
                    s["body"] = st.text_area(f"Slide {i} body", value=s.get("body", ""), key=f"sb_{deck['id']}_{i}", height=80)
