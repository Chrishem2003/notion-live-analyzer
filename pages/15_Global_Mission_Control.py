...
def render_weather_tab():
    ...
    if st.button(" Execute Meteorological Query", key="mc_weather_fetch_upg", type="primary"):
        ...
        if result.get("error"):
            ...
            c1.metric("Current Temperature", f"{result.get('temperature_c', 0)} Â°C")
            c2.metric("Wind Speed", f"{result.get('windspeed_kmh', 0)} km/h")
            c3.metric("Wind Direction", f"{result.get('wind_direction', 0)}Â°")
            ...
            if forecast:
                ...
                df_fc = pd.DataFrame({
                    ...
                    "Min Temperature (Â°C)": forecast.get("temperature_2m_min", []),
                ...
                if PLOTLY_AVAILABLE:
                    fig = go.Figure()
                    fig.add_trace(go.Scatter(x=df_fc["Date"], y=df_fc["Max Temperature (Â°C)"], name="Max Temp", line=dict(color="#00f2fe", width=2)))
                    fig.add_trace(go.Scatter(x=df_fc["Date"], y=df_fc["Min Temperature (Â°C)"], name="Min Temp", line=dict(color="#4facfe", width=2)))
        ...
        with cols[i % 3]:
            st.markdown(
                ...
                <div style="background:#0b1321; border:1px solid #00f2fe44; border-radius:12px; padding:1.2rem; margin-bottom:1rem; text-align:center;">
                    <div style="font-size:2.2rem;">{sector.get('icon', 'âš¡')}</div>
