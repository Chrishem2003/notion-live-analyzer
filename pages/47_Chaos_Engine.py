...
sector = st.sidebar.selectbox(
    "2. Select Institutional Sector / Problem",
    [
...
user_role = st.sidebar.selectbox(
    "3. Select User View Mode",
    [
        "👔 Plain-English Executive (Storyboard & AI Prescriptions)", 
...
st.sidebar.markdown("---")
st.sidebar.markdown("### ⚙️ System Situation Sliders")
...
if "Education" in sector:
    ...
    b = st.sidebar.slider("Friction / Bottleneck Index ($b$)", 0.1, 3.0, 0.8, 0.1)
    c = st.sidebar.slider("Buffer Stability ($c$)", 0.0, 3.0, 1.0, 0.1)

st.sidebar.markdown("---")
policy_shock = st.sidebar.slider("Simulate Crisis Shock Event (Strike, Outbreak, Drought)", -3.0, 3.0, 0.0, 0.1)
...
st.markdown(f'<p class="sub-header">Target Region: <b>{target_country}</b> | Objective: <b>{sector}</b> | Mode: <b>{user_role}</b></p>', unsafe_allow_html=True)
...
# --- View Mode 1: Plain-English Executive Storyboard with AI Prescriptions ---
if "Plain-English Executive" in user_role:
    st.markdown("## 📊 Executive Storyboard & AI Autonomous Prescription")
    
    if mlce_value > 0:
        st.error("🚨 **AUTOMATED SOVEREIGN ALERT DISPATCHED:** Risk threshold exceeded! Emergency notification simulation sent via SMS/Webhook to registered institutional stakeholders.")
    else:
        st.success("🟢 **AUTOMATED TELEMETRY STATUS:** All systems operating within secure homeostatic tolerances.")
    ...
    with col_story1:
        ...
        if "Education" in sector:
            if mlce_value < 0:
                ...
                st.warning("⚠️ **Tuition Collection Bottleneck:** Payment delays combined with structural overhead are producing a cash deficit. Faculty payroll or logistical buffers may require immediate subsidization.")
        elif "Healthcare" in sector:
            if mlce_value < 0:
            ...
            st.info("✅ **Sector Overview:** Operational flows remain stable. Continue monitoring incoming departmental data feeds.")
    ...
    st.markdown("---")
    st.markdown("### 📥 Smart Data Auto-Importer")
    ...
    if uploaded_file is not None:
        try:
            ...
            st.info("💡 **Auto-Mapping Active:** Data linked to sovereign engine.")
...
# --- View Mode 2: Multi-Scenario Policy Comparison Matrix ---
    ...
    st.markdown("Compare how different administrative strategies handle the active crisis shock in real-time:")
    ...
    with col_comp1:
        ...
        ax_comp.set_title("Strategy Performance Under Shock")
    ...
    with col_comp2:
        ...
        st.markdown("""
        * **Option 1 (Do Nothing):** High risk of systemic failure if shock threshold exceeds tolerance.
        * **Option 2 (Emergency Subsidy):** Effectively absorbs short-term shocks by injecting cash buffers. **Recommended for immediate relief.**
        * **Option 3 (Structural Reform):** Lowers friction over the long term, preventing recurring crises.
        """)
        if st.button("🚀 Authorize Option 2 (Emergency Subsidy Deployment)"):
            st.success("✅ Option 2 officially authorized and broadcast to institutional command nodes.")
...
# --- View Modes 3 & 4: Technocrat & Research Scientist ---
    ...
    with tabs[3]:
        ...
        st.markdown(f"""
        * **Target Territory:** {target_country} | **Sector:** {sector}
