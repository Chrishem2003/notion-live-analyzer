# ═══════════════════════════════════════════════════════════════════════════════
# ADVANCED VISUALS STUDIO [ENTERPRISE EDITION - HIGH PREMIUM]
# ═══════════════════════════════════════════════════════════════════════════════
...
# ─── Elite High-Premium Design System Styling ─────────────────────────
st.markdown(
    """
    <style>
    /* Global App Background & Font Render */
    .stApp {
        background: linear-gradient(135deg, #090d16 0%, #0d1117 50%, #111827 100%) !important;
    ...
    span, p, label, .stMarkdown, .stCaption, .streamlit-expanderHeader {
    ...
    /* Premium Glassmorphism Cards & Containers */
    ...
    /* Input Fields, Selectboxes & Sliders Polish */
    ...
    .stSlider tr, .stSlider div {
        color: #3b82f6 !important;
    }

    /* Custom Containers & Dividers */
    hr {
...
if active_df is None or active_df.empty:
  st.warning(
      "⚠️ No active dataset available. Please load data from the File Analyzer"
      " or Notion workspace first."
  )
  st.stop()
...
numeric_cols = [
    c for c in all_columns if col_types.get(c) in ("numeric", "integer")
]
cat_cols = [
    c for c in all_columns if col_types.get(c) in ("categorical", "string")
]
temporal_cols = [c for c in all_columns if col_types.get(c) == "temporal"]

# ─── Global Sidebar / Expandable Dataset Filter & Transformation ──────
with st.expander(
    "🛠️ Optional: Dataset Row Filtering & Slice Controls", expanded=False
):
  ...
  with col_f2:
    if filter_col and filter_col in cat_cols:
      ...
      min_v, max_v = float(active_df[filter_col].min()), float(
          active_df[filter_col].max()
      )
      val_range = st.slider(
...
with tab1:
  ...
  with col_sel1:
    selected_cols = st.multiselect(
        ...
        options=all_columns,
        default=(
            numeric_cols[:2] + cat_cols[:1]
            if numeric_cols
            else all_columns[:3]
        ),
        key="auto_rec_multiselect",
        help=(
            "Engine evaluates distribution, correlation, and cardinality across"
            " columns."
        ),
    )
  with col_sel2:
  ...
  if selected_cols:
    ...
    if recommendations:
      st.markdown(
          f"**Successfully generated {min(len(recommendations), max_recs)}"
          " tailored visualization option(s)**"
      )
      for i, rec in enumerate(recommendations[:max_recs]):
        with st.container():
          ...
          with col_exp:
            ...
            st.caption(
                f"📌 Architecture: `{chart_type.replace('_', ' ').title()}`"
            )
            ...
            rec_fig = build_chart(
                chart_type, active_df, **chart_kwargs, height=320
            )
            if rec_fig:
              dl_link = get_chart_download_link(
                  rec_fig, f"recommended_{chart_type}_{i}", "png"
              )
              if dl_link:
          ...
          with col_chart:
            if rec_fig:
              ...
              st.info(
                  f"⚠️ Could not build `{chart_type}` with the selected column"
                  " parameters."
              )
    else:
      st.info(
          "📭 No recommendations available for this exact column combination."
          " Try selecting a mix of numeric and categorical variables."
      )
  else:
    st.info(
        "👈 Please select at least one column above to trigger automated chart"
        " recommendations."
    )
...
with tab2:
  ...
  st.caption(
      "Full parameter control over chart geometry, mapping axes, color"
  ...
  if st.button(
      "🚀 Render Custom Chart Studio Output",
      type="primary",
      use_container_width=True,
  ):
    if not x_col and not y_col:
      st.warning(
          "⚠️ Please select at least an X-axis or Y-axis column to build the"
          " visualization."
      )
    else:
      with st.spinner("Rendering high-performance interactive plot..."):
        ...
        if fig:
          ...
          with col_ex1:
            png_link = get_chart_download_link(
                fig, f"custom_{chart_type}", "png"
            )
            if png_link:
              st.markdown(png_link, unsafe_allow_html=True)
          with col_ex2:
            html_link = get_chart_download_link(
                fig, f"custom_{chart_type}", "html"
            )
            if html_link:
              st.markdown(html_link, unsafe_allow_html=True)
        else:
          st.error(
              f"❌ Failed to construct '{chart_type}' chart with the selected"
              " parameters. Check column data types."
          )
...
with tab3:
  ...
  st.caption(
      "Describe your analytical objective (e.g., 'distribution', 'time-series"
      " trend', 'correlation', 'hierarchy'), and the engine will map the"
      " optimal chart type."
  )
  ...
  search_query = st.text_input(
      ...
      placeholder=(
          "e.g., compare categories, show composition, trace timeline trend"
      ),
      ...
      for chart_type, desc in results:
        with st.container():
          ...
          if matching_recs:
            ...
            fig_prev = build_chart(
                chart_type, active_df, **sub_kwargs, height=350
            )
            if fig_prev:
              st.plotly_chart(fig_prev, use_container_width=True)
          else:
            st.info(
                f"ℹ️ Quick preview not available for `{chart_type}` with current"
                " dataset schema."
            )
          st.markdown("---")
    else:
      st.warning(
          f"⚠️ No matching chart type found for '{search_query}'. Try keywords"
          " like: trend, distribution, comparison, correlation."
      )
    ...
    with q_cols[i % 4]:
      if st.button(label, use_container_width=True, key=f"quick_btn_{chart_type}"):
        ...
        if matching:
          ...
          st.info(
              f"⚠️ Unable to generate `{chart_type}` automatically with current"
              " data types."
          )
...
with tab4:
  ...
  st.markdown(
      "Simultaneous side-by-side rendering of key dataset metrics and"
      " distributions."
  )
  ...
  if len(numeric_cols) >= 2:
    ...
    with col_d1:
      ...
      fig_d1 = build_chart(
          "histogram", active_df, x=numeric_cols[0], height=350
      )
      if fig_d1:
    ...
    with col_d2:
      st.markdown(
          f"**Secondary Correlation: `{numeric_cols[0]}` vs"
          f" `{numeric_cols[1]}`**"
      )
      fig_d2 = build_chart(
          "scatter",
    ...
    st.info(
        "ℹ️ Multi-chart executive dashboard requires at least 2 numeric"
