...
def render_meetings_hub(conn):
    ...
    with col2:
        st.markdown(f"#### Live Stream Window â€” Room: `{room_name}`")
...
def render_projects(conn):
    ...
    with st.form("new_project_form_upg"):
        ...
        if submitted and proj_name.strip():
            ...
            conn.commit()
            st.success(f"âœ… Project `{proj_name}` successfully initialized and persisted.")
            st.rerun()
...
def render_pipeline(conn):
    ...
    with st.form("new_pipeline_form"):
        ...
        if st.form_submit_button("âž• Add Submission", type="primary") and p_title.strip():
            ...
            conn.commit()
            st.success(f"âœ… Added `{p_title}` to the pipeline.")
            st.rerun()
...
def render_team_workspace(conn):
    ...
    if st.button("ðŸ’¾ Save Roster", key="save_roster_btn"):
        ...
        conn.commit()
        st.success("âœ… Roster successfully updated and synced.")
        st.rerun()
    ...
    if st.button("ðŸ“ Broadcast Note", type="primary", key="save_team_note_btn"):
        if note.strip():
            ...
            conn.commit()
            st.success("âœ… Note broadcast and saved.")
            st.rerun()
...
def render_venture_portfolio():
    section_header(
        ...
        "Real business venture data migrated from an earlier standalone build â€” actual capital allocation "
...
def main():
    ...
    # FIX: this hub had no tier gate at all â€” every trial/free account could
