...
st.set_page_config(page_title="Forensics Intelligence", page_icon="🔍", layout="wide")
...
"""
 Forensics Intelligence â€” Digital Evidence Laboratory (Hardened Production Grade)
...
def render_evidence_lab_tab():
    ...
    if uploaded is None:
        st.info("â„¹ Upload any file artifact to initiate comprehensive bit-level forensic investigation.")
        return
    ...
    st.success(f" Immutable Chain-of-Custody Case Bound: **{case_id}** (this evidence's own case â€” not shared with other uploads this session)")
    try:
    ...
    st.download_button("â¬‡ Download Complete Evidence Dossier (JSON)", data=dossier, file_name=f"evidence_dossier_{case_id}.json", mime="application/json", key="fe_download_upg")


def render_metadata_tab():
    section_header(" Deep Metadata & EXIF Geolocation Forensics", "Extract hidden EXIF tags, device manufacturer signatures, original capture timestamps, and interactive GPS coordinate mapping.")

    uploaded = st.file_uploader("Upload image evidentiary artifact (JPEG / PNG / TIFF)", type=["jpg", "jpeg", "png", "tiff"], key="fe_meta_upload_upg")
    if uploaded is None:
        st.info("â„¹ Upload an image to extract embedded EXIF parameters and geolocation data.")
        return
    ...
    if exif.get("has_exif"):
        ...
        if exif.get("GPS"):
            ...
            if lat is not None and lon is not None:
                ...
                st.info("â„¹ GPS dictionary present, but coordinate values are invalid.")
        else:
            st.info("â„¹ No GPS coordinates embedded in EXIF block.")
    else:
    ...
    if st.button(" Append Extraction Event to Custody Ledger", key="fe_meta_custody_upg"):
        ...
        if case_id:
            ...
            st.warning("âš  No active case found. Ingest this file in the Evidence Lab tab first to open its case.")
...
def render_stego_tab():
    ...
    if uploaded is None:
        st.info("â„¹ Upload an image to perform LSB steganographic profiling.")
        return
...
def render_phishing_tab():
    ...
    if raw_email and st.button(" Execute Email Forensic Analysis", key="fe_analyze_email_upg", type="primary"):
        ...
        if risk == "HIGH":
            st.error(f" **{result.get('verdict', 'Potential Phishing')}** â€” Assessed Risk Level: **{risk}**")
        elif risk == "MEDIUM":
            st.warning(f"âš  **{result.get('verdict', 'Suspicious Headers')}** â€” Assessed Risk Level: **{risk}**")
        else:
...
def render_custody_tab():
    section_header(" Cryptographic Chain-of-Custody Vault", "Court-admissible tamper-evident ledger where every investigative action is cryptographically chained via SHA-256 blocks â€” one independent chain per piece of evidence.")

    registry = st.session_state.get("fe_case_registry", {})
    if not registry:
        st.info("â„¹ No forensic cases opened this session. Ingest an evidentiary artifact in the Evidence Lab to initialize a case.")
        return
    ...
    if st.button("âœ… Verify Cryptographic Ledger Integrity", key="fe_verify_chain_upg", type="primary"):
        ...
        if result.get("valid"):
            st.success(f" Chain integrity verified successfully â€” {result.get('records', 'Unknown')} immutable ledger entries intact for this case.")
        else:
...
def main():
    ...
    setup_page("Forensics Intelligence", " initial_sidebar_state="expanded")
    ...
    hero_card(
        " Forensic Intelligence & Digital Evidence Laboratory â€” Hardened Production Suite",
