...
st.sidebar.markdown(f"### ⚙️ Dynamics — {sector_key}")
...
# ============================================================================
# ENTERPRISE EXTENSION MODULES (SAFE INTEGRATION)
# ============================================================================
try:
    ...
    from sovereign_core.reporting import ExecutiveReportGenerator
    EXTENSIONS_AVAILABLE = True
except Exception:
    EXTENSIONS_AVAILABLE = False

if EXTENSIONS_AVAILABLE:
    if "Neural ODE" in user_role or "Advanced" in user_role:
        st.markdown("### 🧠 Advanced Neural ODE & PINN Physics Core")
        st.caption("Continuous-time machine learning predictions with physics conservation validation.")
        
        neural_engine = NeuralODEEngine()
        x_node, y_node, z_node = neural_engine.forecast_latent(initial_state, t, policy_shock)
        pinn_residual = PINNValidator.validate_conservation(x_node, y_node, z_node)
        
        col_ml1, col_ml2 = st.columns(2)
        with col_ml1:
            st.metric("Neural ODE Latent Divergence", f"{np.mean(x_node):.4f}")
        with col_ml2:
            st.metric("PINN Conservation Residual", f"{pinn_residual:.6f}")

    if "Live Data" in user_role or "Ingestion" in user_role:
        st.markdown("### 📡 Live API Telemetry & Streaming Feed")
        live_yield = LiveDataConnector.fetch_market_yields(target_country)
        live_icu = LiveDataConnector.fetch_icu_capacity(specific_location)
        live_crop = LiveDataConnector.fetch_satellite_crop_index()
