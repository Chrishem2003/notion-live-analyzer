...
def _render_featured(engine: OpportunityFeedEngine, country: str):
    ...
    if not featured:
        return
    st.markdown("### â­ Featured for You")
        ...
        with cols[i]:
            ...
            st.markdown(
                ...
                <div style="background:#171B23;border:1px solid #6366f155;border-radius:14px;padding:1rem;height:100%;">
                    ...
                    <div style="color:#6B7280;font-size:0.75rem;">{amount or 'Amount varies'} Â· {opp.get('field_of_study','')}</div>
                </div>
                """,
...
def render_opportunity_feed_tab():
    """Main entry point â€” render the full Opportunity Feed tab."""
    st.markdown(
        "### ðŸŽ“ Scholarship & Opportunity Feed\n"
    ...
    with col_f4:
        query = st.text_input("ðŸ” Search opportunities", placeholder="e.g. 'AI', 'Africa', 'Masters'", key="opp_query")

    # â”€â”€ Fetch feed â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    for opp in results:
        ...
        with st.container():
                        ...
                        <div style="flex:1;min-width:0;">
                            <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:0.5rem;">
                                <div>
                                    ...
                                    <div style="color:#8b93a8;font-size:0.78rem;">{opp.get('organization','')} Â· {opp.get('source_authority','')}</div>
                                </div>
                            ...
                            <div style="display:flex;flex-wrap:wrap;gap:0.35rem;margin:0.4rem 0;">
                                ...
                                <span class="feed-badge" style="background:rgba(34,197,94,.15);color:#4ade80;border:1px solid rgba(34,197,94,.3);border-radius:999px;padding:0.05rem 0.5rem;font-size:0.65rem;">ðŸ’° {amount or 'Amount varies'}</span>
                                <span class="feed-badge" style="background:rgba(59,130,246,.15);color:#60a5fa;border:1px solid rgba(59,130,246,.3);border-radius:999px;padding:0.05rem 0.5rem;font-size:0.65rem;">ðŸŒ {opp.get('region','')}</span>
                            ...
                            <div style="color:#6B7280;font-size:0.78rem;line-height:1.5;">{opp.get('description','')[:220]}{'â€¦' if len(opp.get('description',''))>220 else ''}</div>
            ...
            with b1:
                if st.button("â­ Save to Pipeline", key=f"save_{opp['id']}", width='stretch'):
