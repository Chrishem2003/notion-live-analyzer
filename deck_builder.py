...
def render_deck_builder_ui():
    ...
    with col2:
        if st.button("ðŸ—‘ï¸ Clear Deck", type="secondary", width='stretch'):
            deck.clear_deck()
    ...
    if add_method == "Current Data/Charts":
        ...
        with col1:
            if st.button(" Add Data Table Slide", width='stretch'):
                ...
                if df is not None and not df.empty:
                    ...
                    st.rerun()
        with col2:
            if st.button("ðŸ“ˆ Add Chart Slides", width='stretch'):
        ...
        with col3:
            if st.button("ðŸ“‹ Add Dataset Profile", width='stretch'):
                df = st.session_state.get("active_df")
                if df is not None:
        ...
        with st.form("custom_slide_form"):
            ...
            submitted = st.form_submit_button("âž• Add Custom Slide", width='stretch')
    ...
    with col_prev:
        if st.button("â¬…ï¸ Previous", width='stretch', disabled=deck.current_slide_idx <= 0):
    ...
    with col_counter:
        ...
        if slide_idx != current:
            ...
            st.rerun()
    with col_next:
        if st.button("Next âž¡ï¸", width='stretch', disabled=deck.current_slide_idx >= len(deck.slides) - 1):
    ...
    with col_edit1:
        if st.button("â¬†ï¸ Move Up", width='stretch', disabled=deck.current_slide_idx == 0):
            ...
            st.rerun()
    with col_edit2:
        if st.button("â¬‡ï¸ Move Down", width='stretch', disabled=deck.current_slide_idx >= len(deck.slides) - 1):
            idx = deck.current_slide_idx
            deck.slides[idx], deck.slides[idx + 1] = deck.slides[idx + 1], deck.slides[idx]
            deck.current_slide_idx = min(len(deck.slides) - 1, idx + 1)
            st.rerun()
    with col_edit3:
        if st.button("ðŸ—‘ï¸ Remove Slide", width='stretch', type="secondary"):
    ...
    with col_exp1:
        if st.button("ðŸŒ Export as HTML", width='stretch'):
    ...
    with col_exp3:
        if st.button("ðŸ“‹ Copy Deck Summary", width='stretch'):
