...
def render_secure_vault_ui():
        ...
        with col2:
            ...
            if setup_mode:
                ...
                with st.form("vault_setup_form"):
                    ...
                    with col_a:
                        submitted = st.form_submit_button("ðŸ” Create Vault", type="primary", width='stretch')
                    with col_b:
                ...
                with st.form("vault_unlock_form"):
                    ...
                    submitted = st.form_submit_button("ðŸ”“ Unlock Vault", type="primary", width='stretch')
                    if submitted:
                        if vault is None:
    ...
    with action_col1:
        if st.button("ðŸ”’ Lock Vault", width='stretch'):
            vault.lock()
            st.session_state["vault_unlocked"] = False
            st.rerun()
    with action_col2:
    ...
    for i, tab_name in enumerate(tab_options):
        with tab_cols[i]:
            ...
            if st.button(tab_name, key=f"vault_tab_{tab_name}", width='stretch', type=btn_type):
    ...
    if not files:
        ...
        for vf in files:
            ...
            with col_view:
                if st.button("ðŸ‘ï¸", key=f"vault_view_{vf.id}", width='stretch', help="Preview file"):
                    st.session_state["vault_preview_file"] = vf.id
                    st.rerun()

            with col_share:
                if st.button("ðŸ”—", key=f"vault_share_{vf.id}", width='stretch', help="Generate share link"):
                    link = vf.generate_share_link(expires_in_hours=1, max_downloads=1)
                    st.info(f"ðŸ”— Share link: `{link['url']}`  expires in 1 + h / 1 download")

            with col_del:
                if vf.is_deleted:
                    if st.button("â™»ï¸", key=f"vault_restore_{vf.id}", width='stretch', help="Restore from trash"):
                        vault.restore_file(vf.id)
                        st.rerun()
                else:
                    if st.button("ðŸ—‘ï¸", key=f"vault_del_{vf.id}", width='stretch', help="Move to trash"):
    ...
    with st.expander("ðŸ“¤ Upload Files to Vault", expanded=False):
        ...
        if uploaded_file:
            ...
            if st.button("ðŸ” Encrypt & Upload", type="primary", width='stretch'):
                try:
