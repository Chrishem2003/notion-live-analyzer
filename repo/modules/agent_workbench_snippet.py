...
Predictive Studio both make sense — it uses `get_active_dataframe()` from
...
st.markdown("### 🦾 Live Agent Workbench")
st.caption(
...
run_swarm_btn = st.button("🚀 Deploy Agent Swarm", type="primary", key="run_swarm_btn")
...
if run_swarm_btn:
    if not problem.strip():
        ...
        research_box.info("🔎 Research agent: querying CrossRef...")
        audit_box.info("📊 Data auditor: profiling active dataset..." if df is not None else "📊 Data auditor: no dataset loaded — will report that honestly.")
        synthesis_box.info("🧠 Synthesis agent: waiting on both agents above...")
        ...
        for step in graph.stream({
            ...
            if node_name == "research":
                ...
                if lit:
                    research_box.success(f"✅ Research agent: found {lit['n_found']} papers (top: \"{lit['top_papers'][0]['title'][:70]}...\")" if lit["top_papers"] else f"✅ Research agent: 0 matches for this query.")
                else:
                    research_box.error(f"❌ Research agent failed: {node_output.get('errors', ['unknown error'])[-1]}")

            elif node_name == "audit":
                audit = node_output.get("audit_findings")
                if audit and audit.get("status") == "ok":
                    audit_box.success(f"✅ Data auditor: {audit['n_rows']} rows, {audit['missing_cells']} missing cells, "
                                       f"{len(audit['numeric_columns_with_outliers'])} column(s) with outliers.")
                elif audit:
                    audit_box.warning(f"⚠️ Data auditor: {audit.get('note', audit.get('status'))}")
                else:
                    audit_box.error(f"❌ Data auditor failed: {node_output.get('errors', ['unknown error'])[-1]}")

            elif node_name == "synthesis":
                synthesis_box.success("✅ Synthesis agent complete.")

        st.markdown("---")
        st.markdown("#### 📋 Synthesis Output")
        st.markdown(final_state.get("synthesis", "_No synthesis produced — check agent errors above._"))

        if final_state.get("errors"):
            with st.expander("⚠️ Agent errors this run"):
