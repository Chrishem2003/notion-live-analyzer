  ...
  with col2:
    ...
    st.markdown(
        ...
        <div class="landing-container">
            ...
            <div class="hub-subtitle">Sovereign Enterprise Engine • Advanced Analytics & Bioinformatics Gateway</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with tab_signin:
      ...
      with c_b1:
        if st.button("🚀 Authenticate Hub", use_container_width=True):
          ...
          st.session_state.user_identity = {
              ...
              "name": entered_name,
              "role": (
                  "Supreme Architect & System Owner"
                  if is_admin
                  else "Verified Enterprise Analyst"
              ),
              "is_admin": is_admin,
          }
          st.rerun()
      ...
      with c_b2:
        if st.button(
            "🔵 Instant Google Sign-In (Simulated)", use_container_width=True
        ):
          st.session_state.portal_unlocked = True
          st.session_state.user_identity = {
    ...
    with tab_signup:
      ...
      if st.button(
          "✨ Register & Launch Sovereign Engine", use_container_width=True
      ):
        ...
        st.session_state.user_identity = {
            ...
            "role": (
                "Supreme Architect & System Owner"
                if is_admin
                else "Registered Pioneer Analyst"
            ),
            "is_admin": is_admin,
        }
        st.rerun()
    ...
    with d_col1:
      st.download_button(
          "💻 Windows (.exe)",
          data=b"Mock Windows Compiled Enterprise Bundle Binary",
          file_name="ChrishemScienceHub_Win64.zip",
          use_container_width=True,
      )
    with d_col2:
      st.download_button(
          "🍏 macOS (.app/.dmg)",
          data=b"Mock macOS Universal Binary Package",
          file_name="ChrishemScienceHub_macOS.dmg",
          use_container_width=True,
      )
    with d_col3:
      st.download_button(
          "🐧 Linux (.AppImage)",
          data=b"Mock Linux Standalone AppImage Binary",
          file_name="ChrishemScienceHub_Linux.AppImage",
          use_container_width=True,
      )
    with d_col4:
      st.download_button(
          "📱 Mobile (.apk)",
          data=b"Mock Android Compiled Client Package",
          file_name="ChrishemScienceHub_Mobile.apk",
          use_container_width=True,
      )
