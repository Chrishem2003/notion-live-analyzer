...
def getlincoef(e, xset):  # e = a*xb ; x in xset
    """
    Obtain ``a`` and ``b`` when ``e == "a*xb"``, where ``x`` is a symbol in
    xset.
...
_is_kind_number = re.compile(r'\d_').match
