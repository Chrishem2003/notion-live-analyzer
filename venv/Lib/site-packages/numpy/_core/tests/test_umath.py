...
class TestDivision:
    ...
    @pytest.mark.parametrize("dtype,ex_val", itertools.product(
        sctypes['int']  sctypes['uint'], (
            ...
            (
                # dividend
                "np.arange(fo.min, fo.minlsize).astype(dtype),"
                # divisors
                ...
                "np.arange(lsize).astype(dtype),"
                # scalar divisors
                "[1,3,9,13,neg, fo.min1, fo.min//2, fo.max//3, fo.max//4]"
            )
        )
    ))
    ...
    def test_division_complex(self):
        ...
        x = np.array([1.e110, 1.e-110], dtype=np.complex128)
        y = x**2 / x
...
class TestPower:
    ...
    def test_power_fast_paths(self):
        ...
        for dt in [np.float32, np.float64]:
            ...
            expected = np.array([0.0, 1.21, 4., 1.44e26, 100, np.inf, np.inf])
            result = np.power(a, 2.)
