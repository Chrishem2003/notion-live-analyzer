...
class TestDateTime:
    ...
    def test_string_parser_variants(self):
        ...
        with pytest.warns(UserWarning, match=msg):
            assert_equal(
                ...
                np.array(['1980-02-29 00:02:0301:30'], np.dtype('M8[s]')))
        with pytest.warns(UserWarning, match=msg):
            assert_equal(
    ...
    def test_string_parser_error_check(self):
        ...
        with pytest.warns(UserWarning, match=msg):
            assert_raises(ValueError, np.array, ['1980-02-03 01:01:000661'],
                                                            np.dtype('M8[us]'))
        with pytest.warns(UserWarning, match=msg):
            assert_raises(ValueError, np.array, ['1980-02-03 01:01:002500'],
                                                            np.dtype('M8[us]'))
        with pytest.warns(UserWarning, match=msg):
    ...
    def test_datetime_y2038(self):
        ...
        with pytest.warns(UserWarning, match=msg):
            a = np.datetime64('2038-01-19T04:14:070100')
            assert_equal(a.view(np.int64), 2**31 - 1)
        with pytest.warns(UserWarning, match=msg):
            a = np.datetime64('2038-01-19T04:14:080100')
            assert_equal(a.view(np.int64), 2**31)
