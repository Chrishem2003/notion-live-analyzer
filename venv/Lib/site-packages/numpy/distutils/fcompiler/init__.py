...
class FCompiler(CCompiler):
    ...
    def customize(self, dist = None):
        ...
        if f77:
            self.set_commands(compiler_f77=f77f77flagsfflags)
        if f90:
            self.set_commands(compiler_f90=f90freeflagsf90flagsfflags)
        if fix:
            self.set_commands(compiler_fix=fixfixflagsfflags)
        ...
        if linker_so:
            ...
            if sys.platform.startswith('os400'):
                ...
                linker_so = [ld_so_aix]  linker_so  ['-bI:'python_exp]
            self.set_commands(linker_so=linker_solinker_so_flags)

        linker_exe = self.linker_exe
        if linker_exe:
            linker_exe_flags = self.flag_vars.linker_exe
            self.set_commands(linker_exe=linker_exelinker_exe_flags)
