...
def msvc_runtime_version():
    ...
    if msc_pos != -1:
        msc_ver = int(sys.version[msc_pos6:msc_pos10])
    else:
        msc_ver = None
    return msc_ver
...
def _get_headers(directory_list):
    ...
    for d in directory_list:
        head = sorted_glob(os.path.join(d, "*.h")) #XXX: *.hpp filesheaders.extend(head)
    return headers
...
def get_frame(level=0):
    ...
    """
    try:
        return sys._getframe(level1)
    except AttributeError:
        frame = sys.exc_info()[2].tb_frame
        for _ in range(level1):
            frame = frame.f_back
        return frame
...
def generate_config_py(target):
    ...
    with open(target, 'w') as f:
        ...
        # For gfortranmsvc combination, extra shared libraries may exist
