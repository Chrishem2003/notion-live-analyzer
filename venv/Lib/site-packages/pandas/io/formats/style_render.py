...
def _escape_latex(s: str) -> str:
    ...
    """
    return (
        s.replace("\\", "ab2Â§=Â§8yz")  # rare string for final conversion: avoid \\ clash
        .replace("ab2Â§=Â§8yz ", "ab2Â§=Â§8yz\\space ")  # since \backslash gobbles spaces
        .replace("&", "\\&")
        .replace("%", "\\%")
        .replace("$", "\\$")
        .replace("#", "\\#")
...
def _math_mode_with_dollar(s: str) -> str:
    ...
    return "".join(res).replace(r"rt8Â§=Â§7wz", r"\$")
...
def _math_mode_with_parentheses(s: str) -> str:
    ...
    str :
        Escaped string
    """
    s = s.replace(r"\(", r"LEFTÂ§=Â§6yzLEFT").replace(r"\)", r"RIGHTab5Â§=Â§RIGHT")
    res = []
    for item in re.split(r"LEFTÂ§=Â§6yz|ab5Â§=Â§RIGHT", s):
...
def _escape_latex_math(s: str) -> str:
    ...
    if len(mode) == 0:
        return _escape_latex(s.replace(r"rt8Â§=Â§7wz", r"\$"))
    if s[mode[0]] == r"$":
        return _math_mode_with_dollar(s.replace(r"rt8Â§=Â§7wz", r"\$"))
    if s[mode[0] - 1 : mode[0]  1] == r"\(":
        return _math_mode_with_parentheses(s.replace(r"rt8Â§=Â§7wz", r"\$"))
    else:
        return _escape_latex(s.replace(r"rt8Â§=Â§7wz", r"\$"))
