...
def test_isnumeric_unicode(method, expected, any_string_dtype):
    ...
    # 0xFF13: ï¼“ Em 3  # noqa: RUF003
    ser = Series(
        ["A", "3", "Â³", "Â¼", "â˜…", "á¸", "ï¼“", "four"],  # noqa: RUF001
        dtype=any_string_dtype,
    )
    expected_dtype = (
        "bool" if is_object_or_nan_string_dtype(any_string_dtype) else "boolean"
    ...
    if (
        ...
        # pyarrow 21 considers Â¼ and á¸ as a digit, while python does not
...
def test_isnumeric_unicode_missing(method, expected, any_string_dtype):
    values = ["A", np.nan, "Â¼", "â˜…", np.nan, "ï¼“", "four"]  # noqa: RUF001
...
@pytest.mark.parametrize(
    "form, expected",
    [
        ("NFKC", ["ABC", "ABC", "123", np.nan, "ã‚¢ã‚¤ã‚¨"]),
        ("NFC", ["ABC", "ï¼¡ï¼¢ï¼£", "ï¼‘ï¼’ï¼“", np.nan, "ï½±ï½²ï½´"]),  # noqa: RUF001
    ],
)
def test_normalize(form, expected, any_string_dtype):
    ser = Series(
        ["ABC", "ï¼¡ï¼¢ï¼£", "ï¼‘ï¼’ï¼“", np.nan, "ï½±ï½²ï½´"],  # noqa: RUF001
        index=["a", "b", "c", "d", "e"],
        dtype=any_string_dtype,
    )
...
def test_normalize_index():
    idx = Index(["ï¼¡ï¼¢ï¼£", "ï¼‘ï¼’ï¼“", "ï½±ï½²ï½´"])  # noqa: RUF001
    expected = Index(["ABC", "123", "ã‚¢ã‚¤ã‚¨"])
