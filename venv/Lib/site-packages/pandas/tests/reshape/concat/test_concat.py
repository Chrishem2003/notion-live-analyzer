...
def test_concat_null_object_with_dti():
    ...
    dti = pd.DatetimeIndex(
        ["2021-04-08 21:21:1400:00"], dtype="datetime64[ns, UTC]", name="Time (UTC)"
    )
...
def test_concat_none_with_timezone_timestamp():
    ...
    df2 = DataFrame([{"A": pd.Timestamp("1990-12-20 00:00:0000:00")}])
    result = concat([df1, df2], ignore_index=True)
    expected = DataFrame(
        {"A": [None, pd.Timestamp("1990-12-20 00:00:0000:00")]}, dtype=object
    )
    tm.assert_frame_equal(result, expected)
