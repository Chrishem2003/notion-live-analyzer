...
"""
    ...
    def test_to_csv_default_encoding(self, temp_file):
        # GH17097
        df = DataFrame({"col": ["AAAAA", "Ã„Ã„Ã„Ã„Ã„", "ÃŸÃŸÃŸÃŸÃŸ", "èžèžèžèžèž"]})
...
"""
    ...
    def test_to_csv_binary_handle(self, mode, temp_file):
        ...
        with open(temp_file, mode="wb") as handle:
    ...
    def test_to_csv_encoding_binary_handle(self, mode, temp_file):
        ...
        content = "a, b, ðŸŸ".encode("utf-8-sig")
...
def test_unicode_format():
    df = DataFrame({"A": [1.23, 4.56]})
    result = df.to_csv(float_format="{:.2f}â‚¬", encoding="utf-8", lineterminator="\n")
    expected = ",A\n0,1.23â‚¬\n1,4.56â‚¬\n"
    assert result == expected
