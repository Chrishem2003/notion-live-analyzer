...
def test_fy5253_last_onoffset():
    ...
    ts = Timestamp("1984-05-28 06:29:43.9559113540200", tz="Europe/San_Marino")
...
def test_fy5253_nearest_onoffset():
    ...
    ts = Timestamp("2032-07-28 00:12:59.0357294190000", tz="Africa/Dakar")
    fast = offset.is_on_offset(ts)
    slow = (ts  offset) - offset == ts
    assert fast == slow
...
def test_fy5253qtr_onoffset_last():
    ...
    ts = Timestamp("2011-01-26 19:03:40.3310961290200", tz="Africa/Windhoek")
    slow = (ts  offset) - offset == ts
    fast = offset.is_on_offset(ts)
    assert fast == slow
