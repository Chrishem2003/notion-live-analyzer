...
def test_resample_origin_with_tz(unit):
    ...
    resampled = ts.resample("5min", origin="1999-12-31 23:57:0000:00").mean()
    tm.assert_index_equal(resampled.index, exp_rng)

    # origin of '1999-31-12 12:02:0003:00' should be equivalent for this case
    resampled = ts.resample("5min", origin="1999-12-31 12:02:0003:00").mean()
    tm.assert_index_equal(resampled.index, exp_rng)
    ...
    ts = Series(np.random.default_rng(2).standard_normal(len(rng)), index=rng)
    with pytest.raises(ValueError, match=msg):
        ts.resample("5min", origin="12/31/1999 23:57:0003:00").mean()


def test_resample_origin_epoch_with_tz_day_vs_24h(unit):
    # GH 34474
    start, end = "2000-10-01 23:30:000500", "2000-12-02 00:30:000500"
    rng = date_range(start, end, freq="7min").as_unit(unit)
    ...
    start, end = "2000-10-01 23:30:000200", "2000-12-02 00:30:000200"
    rng = date_range(start, end, freq="7min").as_unit(unit)
...
def test_resample_across_dst():
    ...
    # (note that DST happens at 03:0002:00 -> 02:0001:00)
    # 2016-10-30 02:23:0002:00, 2016-10-30 02:23:0001:00
    ...
    # 2016-10-30 02:00:0002:00, 2016-10-30 02:00:0001:00
