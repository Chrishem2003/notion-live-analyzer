...
def git_versions_from_keywords(keywords, tag_prefix, verbose):
    ...
    # no suitable tags, so version is "0unknown", but full hex is still there
    if verbose:
...
def render_pep440(pieces):
    ...
    get a tagged build and then dirty it, you'll get TAG0.gHEX.dirty

    Exceptions:
    1: no tags. git_describe was just HEX. 0untagged.DISTANCE.gHEX[.dirty]
    """
    if pieces["closest-tag"]:
        ...
        # exception #1
        rendered = f"0untagged.{pieces['distance']}.g{pieces['short']}"
        if pieces["dirty"]:
            rendered = ".dirty"
    return rendered
