...
class _BoundaryKind(enum.Enum):
    ...
    AFTER_LOCALS = enum.auto()  # after Vlocal, before V.post0
...
class _BoundaryVersion:
    ...
    ``<=1.0`` includes ``1.0local`` and ``>1.0`` excludes
    ...
    Two kinds exist, shown relative to a base version V::
        ...
        V < Vlocal < AFTER_LOCALS(V) < V.post0 < AFTER_POSTS(V)

    ``AFTER_LOCALS`` sits after V and every Vlocal, but before
...
class Specifier(BaseSpecifier):
    ...
    def _compare_greater_than(self, prospective: Version, spec_str: str) -> bool:
        ...
        # equals V. So >1.0a1 must not match 1.0a1local, but must still
        # match 1.0a2local.
