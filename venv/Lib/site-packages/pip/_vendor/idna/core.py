...
def valid_label_length(label: Union[bytes, str]) -> bool:
    ...
    Per :rfc:`1035` (and :rfc:`5891` Â§4.2.4) a DNS label must not exceed
...
def check_initial_combiner(label: str) -> bool:
    ...
    Per :rfc:`5891` Â§4.2.3.2 a label must not start with a character of
...
def check_hyphen_ok(label: str) -> bool:
    ...
    Per :rfc:`5891` Â§4.2.3.1 a label must not start or end with a hyphen
    (``U002D``), and must not have hyphens in both the third and fourth
...
def valid_contextj(label: str, pos: int) -> bool:
    ...
    ``U200C`` (ZERO WIDTH NON-JOINER, Appendix A.1) and ``U200D``
...
def valid_contexto(label: str, pos: int, exception: bool = False) -> bool:
    ...
    (``U00B7``), Greek lower numeral sign, Hebrew punctuation, Katakana
...
def alabel(label: str) -> bytes:
    ...
    Â§4: the label is validated, Punycode-encoded, and prefixed with
...
def uts46_remap(domain: str, std3_rules: bool = True, transitional: bool = False) -> str:
    ...
    Implements the mapping table from `UTS #46 Â§4
    ...
    for pos, char in enumerate(domain):
        ...
        # UTS #46 Â§4: V is always valid, D is deviation (kept unless transitional),
...
def encode(
    ...
    Splits the input on label separators (only ``U002E`` if ``strict`` is
    set; otherwise also IDEOGRAPHIC FULL STOP ``U3002``, FULLWIDTH FULL
    STOP ``UFF0E``, and HALFWIDTH IDEOGRAPHIC FULL STOP ``UFF61``),
    ...
    :param strict: If ``True``, only ``U002E`` is recognised as a label
        separator.
...
def decode(
    ...
    :param display: If ``True``, any ``xn--`` label that fails IDNA
        ...
        by UTS #46 Â§4 and the WHATWG URL "domain to Unicode" algorithm.
