...
def render_meta_analysis_matrix_ui():
    ...
    with tab1:
        ...
        if lit_papers:
            ...
            if st.button("ðŸ”¨ Build Matrix from Literature Papers", type="primary", width='stretch'):
        ...
        with st.expander("âœï¸ Or enter paper details manually"):
            ...
            if st.button("âž• Add Paper to Matrix", width='stretch') and title:
                paper_data = {
            ...
            if "error" not in stats:
                ...
                if "Citations" in df.columns and not df["Citations"].isna().all():
                    try:
                        ...
                        st.dataframe(top_cited, width='stretch', hide_index=True)
                    except Exception:
                        pass
